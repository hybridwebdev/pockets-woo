...
echo "<p>I am a product</p>";

return;

?>
    <rinnai-app class='grid-card-4 bg-white p-4 gap-1 product'>
                ...
                <?php if( $data['product_type'] =='simple') { ?>
                    ...
                    >
                        ...
                        <button
                            ...
                            @click='
                                state.busy = true;
                                rinnai.cart.addItem( { addItem: state.item } ).then( e => {
                                    state.item.quantity = 1;
                                    state.busy = false;
                                    rinnai.cart.show(3000);
                                    rinnai.toast.success("Item Added");
                                } )
                            '
                        >
                            Add to cart
